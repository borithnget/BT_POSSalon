...
namespace jotun.Controllers
{
    public class ProductController : Controller
    {
        ...
        public ActionResult Create(tblProduct pmodel, tblProductByUnit pbumodel, ProductViewModels model)
        {

            using (jotunDBEntities db = new jotunDBEntities())
            {

                //  var unit = (from p in db.tblUnits
                //             where (p.UnitNameEng == model.UnitName) || (p.UnitNameKh == model.UnitName)
                //             select p).FirstOrDefault();

                // pmodel.Unitid = unit.Id;
                var category = (from p in db.tblCategories
                                where (p.CategoryNameEng == model.CategoryName) || (p.CategoryNameKh == model.CategoryName)
                                select p).FirstOrDefault();

                //var product =(from t in db.tblProducts where t.Status==1 select )
                if (model.QuantityInStockRetail == null)
                {
                    var zero = 0;
                    model.QuantityInStockRetail = zero.ToString();
                }

                pmodel.CategoryId = category.Id;

                //pmodel.Unitid = (Convert.ToDecimal(model.QuantityInStockRetail) / Convert.ToDecimal(model.QuantityInStock)).ToString();
                pmodel.ProductNo = model.ProductName;
                pmodel.quantity_alert = Convert.ToDouble(model.quantity_alert);
                pmodel.CreatedDate = DateTime.Now;
                pmodel.Status = 1;
               /* if (pmodel.ProductImage == null) { pmodel.ProductImage = "/Images/defualimage.jpg" + model.ProductImage; }
                 else { pmodel.ProductImage = "/Images/" + model.ProductImage; }*/
                pmodel.ProductImage = string.IsNullOrEmpty(pmodel.ProductImage)
                ? "/Images/defaultimage.jpg"
                : "/Images/" + model.ProductImage;

                // History tracking fields
                pmodel.LastAction = "Create";
                pmodel.LastModified = DateTime.Now;
                pmodel.ModifiedBy = User.Identity.Name; // Assuming authentication is enabled
                ...
                if (model.GetDetail != null)
                {
                    ...
                    foreach (ProductViewModelDetailbyUnit item in model.GetDetail)
                    {
                        //if (item.QTY==pmodel.Unitid)
                        if (item.UnitTypeID == pmodel.Unitid)
                        {
                            tblProductByUnit productbyunit = new tblProductByUnit();
                            productbyunit.Id = Guid.NewGuid().ToString();
                            productbyunit.Price = Convert.ToDecimal(item.Price);
                            productbyunit.Cost = Convert.ToDecimal(item.Cost);
                            productbyunit.UnitTypeID = item.UnitTypeID;
                            productbyunit.ProductID = pmodel.Id;
                            productbyunit.TypeDefault = Convert.ToBoolean(1);
